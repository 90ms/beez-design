...
다섯 번째 컴포넌트로 semantic Text 명세와 commonMain 구현을 Experimental 상태로 추가했습니다. 6개 typography role과 4개 foreground tone, 정렬·최대 줄·overflow를 제공하며 임의의 `Color`나 `TextStyle` override는 노출하지 않습니다. 공통 단위/UI 테스트, Light/Dark/alternate brand Desktop visual baseline과 한국어·영어 Catalog 상세 가이드가 원격 CI에서 검증되었으며 실제 플랫폼 보조기술 검증은 아직 남아 있습니다.
...
- [Text 명세](docs/components/text.md)
- [Surface 명세](docs/components/surface.md)
...
The fifth component is an Experimental semantic Text specification and commonMain implementation. It provides six typography roles, four foreground tones, alignment, maximum-line, and overflow controls without exposing arbitrary `Color` or `TextStyle` overrides. Shared unit/UI tests, Light/Dark/alternate-brand Desktop visual baselines, and the bilingual Catalog detail guide pass remote CI; real-platform assistive-technology verification remains pending.
...
- [Text specification](docs/components/text.md)
- [Surface specification](docs/components/surface.md)
