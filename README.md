...
다섯 번째 컴포넌트로 semantic Text 명세와 commonMain 구현을 추가했습니다. 6개 typography role과 4개 foreground tone, 정렬·최대 줄·overflow를 제공하며 임의의 `Color`나 `TextStyle` override는 노출하지 않습니다. Catalog 카드와 상세 가이드는 추가되었고, 현재 상태는 Proposed이며 원격 Catalog와 시각 회귀 검증을 완료한 뒤 Experimental로 전환합니다.
...
The fifth component is a semantic Text specification and commonMain implementation. It provides six typography roles, four foreground tones, alignment, maximum-line, and overflow controls without exposing arbitrary `Color` or `TextStyle` overrides. Its Catalog card and detailed guide are present; it remains Proposed until remote Catalog and visual-regression validation are complete.
