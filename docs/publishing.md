...
현재 저장소의 Gradle scaffold는 compile 및 publish 검증 전이다. 아래 명령과 설정은 publication wiring을 구현하고 toolchain을 확인한 뒤 실행한다.
...
publication 설정이 완료된 뒤 로컬 반복 검증에는 다음 명령을 사용한다.
...
승인된 v* tag      → release validation → signing → 전체 target publish
```
...
- [ ] consumer sample이 실제 artifact를 resolve한다.
...
- publication/signing Gradle plugin의 구체적인 선택
