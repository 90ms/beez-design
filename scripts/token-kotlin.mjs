...
const catalogTestBrandProperties = [
  ["backgroundBrand", "color.semantic.background.brand"],
  ["foregroundOnBrand", "color.semantic.foreground.onBrand"],
  ["strokeFocus", "color.semantic.stroke.focus"],
];

export const renderCatalogTestBrandTokens = (testTable, overrideTable) => {
  const renderedPaths = catalogTestBrandProperties.map(([, path]) => path).sort();
  const overridePaths = [...overrideTable.keys()].sort();
  if (
    renderedPaths.length !== overridePaths.length
    || renderedPaths.some((path, index) => path !== overridePaths[index])
  ) {
    throw new Error(`Catalog Test Brand output mapping is incomplete: ${overridePaths.join(", ")}`);
  }

  const colors = catalogTestBrandProperties.map(([property, path]) => [
    property,
    renderColor(requireToken(testTable, path, "color")),
  ]);

  return `// Generated by node scripts/generate-tokens.mjs from specification/tokens/themes/test-brand.theme.json.
// Do not edit this file directly. Update the DTCG source and regenerate it.
...
internal fun BeezTokenScheme.withCatalogTestBrand(): BeezTokenScheme = copy(
    colors = colors.copy(
${colors.map(([property, value]) => `        ${property} = ${value},`).join("\n")}
    ),
)
`;
};
